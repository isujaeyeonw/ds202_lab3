...
##moved tidyverse loading up here to hide the conflicts ouput --jm
```
...
dat6 <- happiness %>% select(income,married,happiness) %>%
filter(income %notin% c("Don't know", "No answer", "Not applicable", "Refused"), 
       happiness %notin% c("No answer", "Not applicable"))
dat6 %>% group_by(income,married) %>%
  summarize(n=n()) %>%
  arrange(desc(income))
...
dat6 %>%
  ggplot(aes(x = income, y=married, fill=happiness)) +   geom_tile() +
